...
type cdpJSONReader interface {
	ReadJSON(v interface{}) error
}

func readCDPResponseForID(reader cdpJSONReader, requestID int) (cdpResponse, error) {
	if reader == nil {
		return cdpResponse{}, fmt.Errorf("CDP reader is nil")
	}
	for {
...
func cdpCall(debugPort int, method string, params map[string]any) (map[string]any, error) {
	...
	// 4. 等待响应
	cdpResp, err := readCDPResponseForID(conn, msg.Id)
	if err != nil {
...
func (a *App) getDebugPort(profileId string) (int, error) {
	...
	if browser.NormalizeProfileRuntimeState(profile) != browser.RuntimeRunning || !profile.Running {
...
func formatNetscapeCookies(cookies []CookieInfo) string {
