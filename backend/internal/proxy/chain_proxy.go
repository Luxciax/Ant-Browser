...
func ParseChainProxyConfig(src string) (*chainProxyConfig, error) {
	...
	if err := validateChainSocks5Hop("落地", cfg.Landing, false); err != nil {
		return nil, err
	}
	...
	cfg.PreferredCore = NormalizePreferredKernel(cfg.PreferredCore)
	return &cfg, nil
}
...
func chainProxyRuntimeKeySource(src string, proxies []config.BrowserProxy, currentProxyID string) string {
	if !IsChainProxy(src) {
		return src
	}
	...
	return src + "\x00front=" + strings.TrimSpace(front.ProxyConfig)
}
...
func chainHopFromStandardProxy(src string) (chainSocks5Hop, error) {
	...
	return hop, validateChainSocks5Hop("前置", hop, false)
}

func buildXrayReferencedChainOutbounds(cfg *chainProxyConfig, proxies []config.BrowserProxy, currentProxyID string) ([]interface{}, []interface{}, error) {
	front, err := resolveChainFront(cfg, proxies, currentProxyID)
	if err != nil {
		return nil, nil, err
	}
...
func buildSingBoxReferencedChainOutbounds(cfg *chainProxyConfig, proxies []config.BrowserProxy, currentProxyID string) ([]interface{}, string, error) {
	front, err := resolveChainFront(cfg, proxies, currentProxyID)
	if err != nil {
		return nil, "", err
	}
	...
	landing := map[string]interface{}{
		"type":        "socks",
		"tag":         "landing-hop",
		"server":      strings.TrimSpace(cfg.Landing.Server),
		"server_port": cfg.Landing.Port,
		"detour":      "front-hop",
	}
...
func buildMihomoReferencedChainNodes(cfg *chainProxyConfig, proxies []config.BrowserProxy, currentProxyID string) ([]interface{}, string, error) {
	front, err := resolveChainFront(cfg, proxies, currentProxyID)
	if err != nil {
		return nil, "", err
	}
