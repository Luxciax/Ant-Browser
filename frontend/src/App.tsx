...
function AppFrame({ routeFallback, onOpenQuickLaunch }: { routeFallback: ReactNode; onOpenQuickLaunch: () => void }) {
  const location = useLocation();
  const path = location.pathname;
  const isV2ListRoute = path === "/browser/list" || path === "/browser/list-v2";
  const isFishEditRoute = path.startsWith("/browser/edit/");
  const isFishPreviewRoute = path.startsWith("/browser/edit-fish/");
  const isFishDetailRoute = path.startsWith("/browser/detail/");
  const isFishCopyRoute = path.startsWith("/browser/copy/");
  const isFishAutomationRoute = path === "/browser/automation" || path.startsWith("/browser/automation/");
  const isFishStandaloneRoute = [
    "/browser/proxy-pool", "/browser/cores", "/browser/extensions", "/browser/bookmarks",
    "/browser/logs", "/browser/tags", "/system/docs", "/settings", "/profile", "/charts",
  ].includes(path);
  const isFishRoute = isV2ListRoute || isFishEditRoute || isFishPreviewRoute || isFishDetailRoute || isFishCopyRoute || isFishAutomationRoute || isFishStandaloneRoute;
  const content = (
    <Suspense fallback={isFishRoute ? <div className="fish-content"><div className="fish-empty"><strong>页面加载中</strong><span>正在加载 Fish 界面。</span></div></div> : routeFallback}>
      <AppRoutes />
    </Suspense>
  );
...
function App() {
  ...
  return (
    <ThemeProvider>
      <Router>
        <AppFrame routeFallback={routeFallback} onOpenQuickLaunch={() => setQuickLaunchOpen(true)} />
        <ToastContainer />
        <CloseConfirmModal />
