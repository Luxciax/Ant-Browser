...
function parseChainSocks5Config(proxyConfig: string): ChainSocks5Config | null {
  ...
  const normalizeHop = (raw: unknown, allowProxyConfig: boolean): ChainSocks5HopConfig | null => {
    if (!raw || typeof raw !== 'object') return null
    const hop = raw as Record<string, unknown>
    const advanced = String(hop.proxyConfig || '').trim()
    if (advanced) return allowProxyConfig ? { proxyConfig: advanced } : null
  ...
  try {
    ...
    const first = normalizeHop(parsed.first, true)
    const second = normalizeHop(parsed.second, false)
